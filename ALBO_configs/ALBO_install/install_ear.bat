...
rem %2 - client/employee

@set APP_NAME=%1
@set APP_EAR=%APP_NAME%%INSTALLING_APPS_VERSION_SUFFIX%.ear

if  $%2$==$employee$ goto employee
if  $%2$==$client$ goto client
...
:install
@copy /y %WAR_DIR%\%APP_EAR% %deploy.was.home%\profiles\%WAS_PROFILE%\bin\%APP_EAR%
pushd %deploy.was.home%\profiles\%WAS_PROFILE%\bin

@echo INSTALL APP %APP_NAME%...
@set CMD_OPTIONS=-verbose -node %WAS_SERVERNODE% -server %WAS_SERVERNAME% -distributeApp -appname %APP_NAME%%WAS_INSTALLING_APPS_VERSION_SUFFIX% -defaultbinding.virtual.host default_host -usedefaultbindings
